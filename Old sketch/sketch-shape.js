...
function setup() {
    ...
    flowTexture = createGraphics(128, 128);
}
let lastTexUpdate = 0;
function updateFlowTexture(style) {
    let now = millis();
    if (now - lastTexUpdate > 200) {  // 每200ms更新一次
        lastTexUpdate = now;
        flowTexture.loadPixels();
        let t = now * 0.001;
        for (let x = 0; x < flowTexture.width; x++) {
            for (let y = 0; y < flowTexture.height; y++) {
                let n = noise(x * 0.05, y * 0.05, t);
                let c = lerpColor(style.colors[0], style.colors[1], n);
                flowTexture.set(x, y, c);
            }
        }
        flowTexture.updatePixels();
    }
}
...
function drawAngryBlob(radius, detail = 20, distortion = 0.4) {
...
class MovementPoint {
    ...
    drawEmotionShape() {
        ...
        switch (style.shape) {
            ...
            case "spikyball":
                push();
                fill(lerpColor(style.colors[0], style.colors[1], 0.5));
                noStroke();
                drawAngryBlob(baseSize * 0.6, 20, 0.5);
                pop();
                break;
