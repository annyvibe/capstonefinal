...
class MovementPoint {
    ...
    drawLine(other) {
        ...
        let numStreams = int(map(avgAmp, 0, 1.0, 3, 5, true)); // 股数
        let particlesPerStream = 16;

        let baseSize = map(avgAmp, 0, 1.0, 0.0015, 0.01, true); // 粒子变小
        let alpha = map(speed, 0, 0.002, 50, 255, true); // 保留速度→亮度
        let colorVal = map(speed, 0, 0.002, 50, 255, true);     // 白亮程度

        let twistStrength = 0.006;
        let twistFreq = 2.5;
        let time = millis() * 0.001;

        let axis = p5.Vector.sub(other.pos, this.pos).normalize();
        let up = createVector(0, 1, 0);
        if (abs(axis.dot(up)) > 0.9) up = createVector(1, 0, 0); // 避免方向共线
        let side1 = axis.cross(up).normalize();
        let side2 = axis.cross(side1).normalize();

        for (let s = 0; s < numStreams; s++) {
            let phaseOffset = TWO_PI * s / numStreams;

            for (let i = 0; i < particlesPerStream; i++) {
                let t = i / (particlesPerStream - 1);
                let center = p5.Vector.lerp(this.pos, other.pos, t);

                let angle = t * twistFreq * TWO_PI + time + phaseOffset;
                let radialOffset = p5.Vector.add(
                    side1.copy().mult(cos(angle)),
                    side2.copy().mult(sin(angle))
                ).mult(twistStrength);

                let finalPos = p5.Vector.add(center, radialOffset);

                push();
                translate(finalPos.x, finalPos.y, finalPos.z);
                noStroke();
                fill(colorVal, alpha);
                sphere(baseSize, 5, 5);  // 粒子减小，细腻
                pop();
            }
        }
    }
